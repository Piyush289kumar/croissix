...
const Icon = {
  ...
  security: (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
      <path d="M12 3L4 7v5c0 5.25 3.5 10.15 8 11.35C16.5 22.15 20 17.25 20 12V7L12 3z" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
...
/* ── Section row ────────────────────────────────────────────── */
function Row({
  icon, iconBg, label, value, destructive = false, isDark,
  onClick,
}: {
  icon: React.ReactNode; iconBg: string; label: string;
  value?: string; destructive?: boolean; isDark: boolean;
  onClick?: () => void;
}) {
...
/* ── Section container ──────────────────────────────────────── */
function Section({ children, isDark }: { children: React.ReactNode; isDark: boolean }) {
  return (
    <div style={{
      ...
      <div style={{ maxWidth: 430, margin: "0 auto", paddingBottom: 100 }}>
        ...
        <div style={{
          ...
          <div style={{ textAlign: "center" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 7 }}>
              <span style={{
                ...
                letterSpacing: "-0.03em", color: titleClr,
              }}>
                User Name
              </span>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 11, fontWeight: 600, color: "#25d366", letterSpacing: "0.05em", textTransform: "uppercase", marginBottom: 3 }}>
              About
            </div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <span style={{ color: "#3b82f6", cursor: "pointer", marginLeft: 8 }}>
                {Icon.edit}
              </span>
            </div>
          </div>
          ...
          <button style={{
            ...
            <span style={{
              ...
              color: "#25d366", letterSpacing: "-0.01em",
            }}>
              View QR Code
            </span>
            <span style={{ marginLeft: "auto", color: "#25d366", opacity: 0.6 }}>
              {Icon.chevron}
            </span>
          </button>
        </div>
        ...
        <div style={{ padding: "16px 16px 0", display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <Section isDark={isDark}>
            <Row icon={Icon.notification} iconBg="#f97316" label="Notifications" isDark={isDark} />
            <Row icon={Icon.privacy}      iconBg="#8b5cf6" label="Privacy"       isDark={isDark} />
            <Row icon={Icon.security}     iconBg="#3b82f6" label="Security"      value="2-step off" isDark={isDark} />
          </Section>

          {/* Chats & Storage */}
          <Section isDark={isDark}>
            <Row icon={Icon.chats}   iconBg="#25d366" label="Chats"   isDark={isDark} />
            <Row icon={Icon.storage} iconBg="#06b6d4" label="Storage and Data" value="1.2 GB" isDark={isDark} />
          </Section>

          {/* Help */}
          <Section isDark={isDark}>
            <Row icon={Icon.help}   iconBg="#64748b" label="Help"          isDark={isDark} />
            <Row icon={Icon.invite} iconBg="#10b981" label="Invite a Friend" isDark={isDark} />
            <Row
              icon={Icon.theme}
          ...
          <Section isDark={isDark}>
            <Row
              icon={Icon.logout} iconBg="rgba(239,68,68,0.15)"
              label="Log Out" destructive isDark={isDark}
            />
          </Section>

          {/* App version footer */}
          <div style={{ textAlign: "center", padding: "8px 0 4px" }}>
            <div style={{ fontSize: 12, color: subClr }}>Croissix · Version 0.1</div>
            <div style={{ fontSize: 11, color: subClr, marginTop: 2, opacity: 0.6 }}>from Vipprow</div>
          </div>
        </div>
      </div>
    </div>
