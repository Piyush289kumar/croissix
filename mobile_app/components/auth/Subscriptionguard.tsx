...
const RZP_KEY = process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID ?? "rzp_live_placeholder";
...
function loadRzp(): Promise<boolean> {
  return new Promise(res => {
    ...
    s.id = "rzp-sg"; s.src = "https://checkout.razorpay.com/v1/checkout.js";
...
function SubscriptionGate({ dark }: { dark: boolean }) {
  ...
  async function handleSubscribe() {
    ...
    if (!loaded) { setFailMsg("Failed to load payment SDK."); setScreen("failed"); return; }
    ...
    try {
      ...
      setFailMsg(err?.response?.data?.message ?? err?.message ?? "Could not create subscription.");
    ...
    const rzp = new window.Razorpay({
      ...
      name: "Vipprow",
      description: `Agency Plan · ${fmt(effectivePrice)}/mo`,
      image: "/logo.png",
      ...
      handler: async (resp: any) => {
        try {
          ...
          setFailMsg(err?.response?.data?.message ?? err?.message ?? "Verification failed.");
          setScreen("failed");
        }
      },
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen overflow-y-auto"
      ...
      <div className="pointer-events-none fixed top-0 left-1/2 -translate-x-1/2 w-[480px] h-[480px] rounded-full opacity-25 z-0"
      ...
      <div className="relative z-10 w-full max-w-md mx-auto px-4 py-8 flex flex-col gap-5">
        <AnimatePresence mode="wait">
            ...
            <motion.div key="plans"
              ...
              <div className="text-center pt-2">
                ...
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full mb-3"
                  style={{
                    background: dark ? "rgba(245,158,11,0.12)" : "rgba(254,243,199,0.8)",
                  ...
                  <span className="text-[10.5px] font-extrabold uppercase tracking-[0.06em]"
                    style={{ color: PLAN.color }}>7-day free trial</span>
                </div>
                <h1 className="text-[24px] font-black leading-tight mb-1.5"
                  style={{ letterSpacing: "-0.04em", color: textP }}>Unlock Croissix</h1>
              ...
              <div className="rounded-3xl overflow-hidden"
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-[18px] font-black" style={{ letterSpacing: "-0.03em", color: textP }}>Agency</span>
                    ...
                    {/* Animated price */}
                    <div className="text-right shrink-0 ml-2" style={{ minWidth: 80 }}>
                      <AnimatePresence mode="wait">
                        {coupon ? (
                          <motion.div key="disc"
              ...
              <div>
                <motion.button onClick={handleSubscribe} disabled={loading} whileTap={{ scale: 0.975 }}
                  ...
                  style={{ background: PLAN.gradient, boxShadow: "0 10px 32px rgba(217,119,6,0.45)", letterSpacing: "-0.01em" }}>
                  <motion.div className="absolute inset-0"
                    style={{ background: "linear-gradient(90deg,transparent,rgba(255,255,255,0.18),transparent)" }}
                    animate={{ x: ["-100%", "100%"] }} transition={{ duration: 2.2, repeat: Infinity, ease: "linear" }} />
                  {loading ? <RefreshCw size={16} className="animate-spin relative" /> : <Zap size={16} className="relative" />}
                  <span className="relative">
                    {loading ? "Loading…" : `Start Free Trial · ${fmt(effectivePrice)}/mo`}
                  </span>
                </motion.button>
                <p className="text-center text-[10.5px] mt-1.5 font-medium" style={{ color: dark ? "#2d3f58" : "#94a3b8" }}>
            ...
            <motion.div key="success" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
              ...
              <motion.div style={{ position: "relative", width: 100, height: 100, marginBottom: 8 }}>
                ...
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
                  ...
                  style={{
                    position: "absolute", top: 12, left: 12, width: 76, height: 76, borderRadius: "50%",
              ...
              <h1 className="text-[26px] font-black" style={{ letterSpacing: "-0.04em", color: textP }}>You're in! 🎉</h1>
              <p className="text-[13px] font-medium" style={{ color: textS }}>Agency plan activated. Welcome to Croissix!</p>
              <div className="w-full rounded-2xl p-3.5 flex items-center gap-3 text-left"
                style={{
              ...
              <motion.button onClick={() => window.location.reload()} whileTap={{ scale: 0.975 }}
          ...
          {/* ══ FAILED ══ */}
          {screen === "failed" && (
            <motion.div key="failed" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
              ...
              <motion.div initial={{ scale: 0 }} animate={{ scale: 1, x: [0, -5, 5, -2, 2, 0] }}
                ...
                className="rounded-full flex items-center justify-center"
                style={{
                  width: 72, height: 72, background: dark ? "rgba(239,68,68,0.1)" : "rgba(254,226,226,0.8)",
              ...
              {failMsg && (
                <div className="w-full rounded-2xl p-3.5 flex items-start gap-2.5 text-left"
                  ...
                  <p className="text-[12px] font-semibold m-0" style={{ color: dark ? "#fca5a5" : "#991b1b" }}>{failMsg}</p>
                </div>
              )}
