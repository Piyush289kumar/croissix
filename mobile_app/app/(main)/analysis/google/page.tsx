...
interface AnalysisSummary {
  ...
  totalDirections: number; totalConversations: number; totalReviews: number;
  avgRating: number; replyRate: number; totalPosts: number;
}
...
interface AnalysisData {
  success: boolean; range: string; startDate: string; endDate: string;
  summary: AnalysisSummary;
  charts: {
    impressionsByDay: ImpressionDay[]; actionsByDay: ActionDay[];
    impressionBreakdown: ImpressionBreak; ratingDistribution: Record<number, number>;
    callSeries: SeriesPoint[]; websiteSeries: SeriesPoint[]; directionSeries: SeriesPoint[];
  };
...
/* ══════════════════════════════════════════════════════════
   ANIMATED NUMBER
══════════════════════════════════════════════════════════ */
function AnimatedNumber({ value, className, style }: { value: string | number; className?: string; style?: React.CSSProperties }) {
  const str = String(value);
  const suffix = str.endsWith("M") ? "M" : str.endsWith("K") ? "K" : str.endsWith("%") ? "%" : str.includes("★") ? "★" : "";
  ...
  useEffect(() => {
    ...
    const target = suffix === "M" ? raw * 1_000_000 : suffix === "K" ? raw * 1_000 : raw;
    spring.set(target);
  }, [inView]);
...
function StatCard({ label, value, icon, color, sub, trend, isDark }: {
  ...
  return (
    <motion.div variants={staggerItem} whileHover={{ y: -2, transition: { duration: 0.18 } }}
      whileTap={{ scale: 0.97 }}
      ...
      <div className="flex items-end gap-2">
        <AnimatedNumber value={typeof value === "number" ? fmt(value) : value}
...
function ChartCard({ title, subtitle, children, isDark, action }: {
  ...
  return (
    <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-20px" }}
...
/* ══════════════════════════════════════════════════════════
   REVIEW CARD
══════════════════════════════════════════════════════════ */
function ReviewCard({ review, isDark, index }: {
            ...
            <div className="flex gap-0.5">
              {[...Array(5)].map((_, i) => (
                <motion.div key={i} initial={{ scale: 0 }} whileInView={{ scale: 1 }}
                  transition={{ delay: 0.15 + i * 0.04, type: "spring", stiffness: 260 }}
                  viewport={{ once: true }}>
                  <Star size={9} style={{
...
function downloadCSV(data: AnalysisData, range: RangeKey) {
  ...
  for (const d of data.charts.impressionsByDay) rows.push([d.date, String(d.desktop), String(d.mobile)]);
  rows.push(["", "", ""]);
  rows.push(["=== DAILY ACTIONS ===", "", "", ""]);
  rows.push(["Date", "Calls", "Website Clicks", "Directions"]);
  for (const d of data.charts.actionsByDay) rows.push([d.date, String(d.calls), String(d.website), String(d.directions)]);
  const csv = rows.map(r => r.join(",")).join("\n");
  const blob = new Blob([csv], { type: "text/csv" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url; a.download = `google-analysis-${range}-${new Date().toISOString().slice(0, 10)}.csv`; a.click();
...
══════════════════════════════════════════════════════════ */
function RatingRing({ avgRating, isDark }: { avgRating: number; isDark: boolean }) {
  ...
  return (
    <div ref={ref} className="relative w-20 h-20 shrink-0">
      <svg viewBox="0 0 80 80" className="w-full h-full -rotate-90">
        ...
        <motion.circle cx="40" cy="40" r="32" fill="none" stroke="#f59e0b" strokeWidth="8"
          strokeLinecap="round"
          initial={{ strokeDasharray: `0 ${circumference}` }}
          animate={inView ? { strokeDasharray: `${(avgRating / 5) * circumference} ${circumference}` } : {}}
                      ...
                      {activeChart === "area" ? (
                        ...
                        <BarChart data={impData} margin={{ top: 4, right: 4, left: -24, bottom: 0 }} barGap={2}>
                          ...
                          <Bar dataKey="mobile" name="Mobile" fill="#06b6d4" radius={[4, 4, 0, 0]} maxBarSize={12}
                            animationDuration={700} animationEasing="ease-out" animationBegin={100} />
                        </BarChart>
                      )}
                    </ResponsiveContainer>
                  </motion.div>
                ...
                <ResponsiveContainer width="100%" height={180}>
                  <LineChart data={actData} margin={{ top: 4, right: 4, left: -24, bottom: 0 }}>
                    ...
                    <Line type="monotone" dataKey="Website" stroke="#8b5cf6" strokeWidth={2} dot={false}
                      animationDuration={800} animationEasing="ease-out" animationBegin={100} />
                ...
                <ChartCard title="Impression Sources" subtitle="Where customers found you" isDark={isDark}>
                  <div className="flex items-center gap-4">
                    ...
                    <motion.div className="flex flex-col gap-2 shrink-0"
                      ...
                      {pieData.map((d, i) => (
                        <motion.div key={i} variants={staggerItem} className="flex items-center gap-1.5">
                  ...
                  <motion.div className={`grid grid-cols-2 gap-2 mt-3 pt-3 border-t ${isDark ? "border-white/[0.05]" : "border-slate-100"}`}
                    ...
                    {[
                      ...
                      <motion.div key={i} variants={staggerItem} whileHover={{ y: -2, transition: { duration: 0.18 } }}
                        ...
                        <motion.span className="p-1.5 rounded-lg" style={{ background: `${row.color}20`, color: row.color }}
                ...
                <motion.div className="flex flex-col gap-2"
                  ...
                  {[
                    ...
                    <motion.div key={i} variants={staggerItem}
                      ...
                      <motion.span className="p-2 rounded-xl shrink-0" style={{ background: `${row.color}18`, color: row.color }}
