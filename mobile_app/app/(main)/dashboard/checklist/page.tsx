...
function scoreTier(n: number) {
  ...
  if (n >= 750)
    return { tier: "Advanced", sub: "Strong — keep optimising", rank: 4 };
  if (n >= 550)
    return { tier: "Growing", sub: "Good foundation, gaps remain", rank: 3 };
  if (n >= 350)
    return { tier: "Basic", sub: "Several key items need fixing", rank: 2 };
...
function PageSkeleton({ dark }: { dark: boolean }) {
  ...
  return (
    <div
      className={`min-h-screen ${dark ? "bg-[#050d1a]" : "bg-[#eef4ff]"} px-4 pt-6 pb-32 max-w-lg mx-auto`}
    >
      ...
      <div className={`h-32 rounded-2xl ${pulse} mb-4`} />
      {[1, 2, 3].map((i) => (
        <div key={i} className={`h-16 rounded-2xl ${pulse} mb-3`} />
      ))}
    </div>
  );
}
...
function NotConnected({ dark }: { dark: boolean }) {
  ...
  return (
    <div
      className={`min-h-screen flex items-center justify-center px-6 ${dark ? "bg-[#050d1a]" : "bg-[#eef4ff]"}`}
    >
...
function ScanOverlay({ dark, onDone }: { dark: boolean; onDone: () => void }) {
  ...
  return (
    <div
      className={`fixed inset-0 z-[600] flex flex-col items-center justify-center ${dark ? "bg-[#050d1a]" : "bg-[#eef4ff]"}`}
    >
      <div
        className="absolute inset-0 opacity-[0.025] pointer-events-none"
        style={{
          backgroundImage:
            "repeating-linear-gradient(0deg,transparent,transparent 3px,#3b82f6 3px,#3b82f6 4px)",
        }}
      />
      <div className="flex flex-col items-center gap-7 px-8 w-full max-w-xs">
        ...
        <div className="text-center w-full">
          <p
            className={`text-[21px] font-black mb-1 ${dark ? "text-white" : "text-slate-900"}`}
          ...
          <p
            ...
            style={{ color: "#60a5fa", minHeight: 18 }}
          >
        ...
        <div className="w-full">
          <div
            className={`h-1.5 rounded-full overflow-hidden ${dark ? "bg-white/[0.08]" : "bg-blue-100"}`}
          >
            <div
        ...
        <div className="w-full flex flex-col gap-1.5">
          {[
            ...
            <div
              key={i}
              className={`flex items-center gap-2.5 transition-all duration-500 ${p > (i + 1) * 15 ? "opacity-100" : "opacity-25"}`}
            >
              <div
                className={`w-4 h-4 rounded-full flex items-center justify-center transition-all ${p > (i + 1) * 15 ? "bg-blue-500" : dark ? "bg-white/[0.06]" : "bg-blue-100"}`}
              >
    ...
    <div className="flex items-center gap-5">
      <div className="relative shrink-0" style={{ width: 120, height: 120 }}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span
            className="text-[30px] font-black leading-none text-blue-500"
            style={{ letterSpacing: "-0.06em" }}
          >
            {score}
          </span>
          <span
            className={`text-[9px] font-bold mt-0.5 ${dark ? "text-blue-500/50" : "text-blue-400"}`}
          >
            / 1000
          </span>
        </div>
      ...
      <div className="flex flex-col gap-2.5 flex-1 min-w-0">
        <div>
          <div className="flex items-center gap-2 mb-0.5">
            <span
              className="text-[18px] font-black leading-none text-blue-500"
              style={{ letterSpacing: "-0.04em" }}
            >
              {tier.tier}
            </span>
          ...
          <p
            className={`text-[11px] font-medium leading-tight ${dark ? "text-slate-500" : "text-slate-400"}`}
          >
            {tier.sub}
          </p>
        </div>
        ...
        <div
          className={`flex items-center justify-between px-3 py-2 rounded-xl border ${dark ? "bg-blue-500/[0.06] border-blue-500/[0.12]" : "bg-blue-50/80 border-blue-200/60"}`}
        >
          {[
            ...
            <div key={i} className="flex flex-col items-center gap-0.5 flex-1">
              {i > 0 && (
                <div
                  className={`absolute w-px h-7 ${dark ? "bg-blue-500/10" : "bg-blue-200/60"}`}
                />
              )}
              ...
              <span
                className={`text-[8.5px] font-semibold ${dark ? "text-slate-600" : "text-slate-400"}`}
              >
                {r.label}
              </span>
            </div>
            ...
            (t, i) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  {active && (
                    <span className="text-[7.5px] font-black text-blue-500">
                      {t}
                    </span>
                  )}
                </div>
    ...
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex justify-between mb-1">
          <span
            className={`text-[10.5px] font-semibold truncate ${dark ? "text-slate-300" : "text-slate-700"}`}
          >
            {cat.label}
          </span>
          <span
        ...
        <div
          className={`h-1 rounded-full overflow-hidden ${dark ? "bg-white/[0.06]" : "bg-blue-50"}`}
        >
          <div
      ...
      <span
        className={`text-[10px] font-black w-7 text-right shrink-0 ${dark ? "text-slate-500" : "text-slate-400"}`}
      >
        {pct}%
      </span>
    </div>
...
function APIBadge({ field, dark }: { field: GoogleAPIField; dark: boolean }) {
  return (
    <div
      className={`flex items-start gap-2 p-2.5 rounded-xl border text-[10px] font-mono ${dark ? "bg-blue-950/40 border-blue-800/30" : "bg-blue-50/80 border-blue-200/60"}`}
    >
      <div className="flex flex-col gap-1 min-w-0 flex-1">
        <span
          className={`font-bold ${dark ? "text-blue-300" : "text-blue-700"}`}
    ...
    >
      ...
      >
        <div className="flex flex-col items-center gap-1 pt-0.5 shrink-0">
          ...
          {open && (
            <div
              className={`w-px flex-1 min-h-[12px] ${dark ? "bg-white/[0.04]" : "bg-blue-100"}`}
            />
          )}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 mb-2">
            <p
              className={`text-[12.5px] font-bold leading-snug ${dark ? "text-white" : "text-slate-900"}`}
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 flex-wrap">
              <span
                className="flex items-center gap-1 text-[9.5px] font-bold px-2 py-0.5 rounded-full border"
                style={{
                  background: cfg.bg,
                  color: cfg.color,
              ...
              <span
                className={`text-[9.5px] font-black ${dark ? "text-slate-500" : "text-slate-400"}`}
              >
                +{item.points} pts
              </span>
              {currentValue && (
                <span
                  className={`text-[9.5px] font-mono truncate max-w-[120px] ${dark ? "text-blue-400/60" : "text-blue-500/70"}`}
                >
                  {currentValue}
                </span>
              )}
            ...
            <div className="flex items-center gap-1 shrink-0">
              {item.apiField.writable ? (
                <span
                  className={`text-[9px] font-bold ${dark ? "text-blue-500" : "text-blue-400"}`}
                >
                  R/W
                </span>
              ) : (
                <span
                  className={`text-[9px] font-bold flex items-center gap-0.5 ${dark ? "text-slate-600" : "text-slate-400"}`}
                >
                  <Lock size={8} />
                  RO
                </span>
      ...
      {open && (
        <div
          className={`border-t ${dark ? "border-white/[0.04]" : "border-blue-100/60"}`}
        >
          <div
            className={`flex gap-0 border-b ${dark ? "border-white/[0.04]" : "border-blue-100/60"}`}
          >
          ...
          <div className="px-4 py-3.5 flex flex-col gap-3">
            {tab === "what" && (
              <div>
                ...
                <p
                  className={`text-[12px] leading-relaxed ${dark ? "text-slate-300" : "text-slate-700"}`}
                >
                  {item.what}
                </p>
                {item.target && (
                  <div
                    className={`mt-2.5 flex items-center gap-2 px-3 py-2 rounded-xl border ${dark ? "bg-blue-500/[0.07] border-blue-500/20" : "bg-blue-50 border-blue-200/60"}`}
                  >
                    <Target size={11} style={{ color: catColor }} />
                    <div>
                      <span
                        className={`text-[9.5px] font-bold uppercase tracking-wide ${dark ? "text-slate-500" : "text-slate-400"}`}
                      >
                        Target:{" "}
                      </span>
                      <span
                        className={`text-[11px] font-bold ${dark ? "text-white" : "text-slate-900"}`}
                      >
                        {item.target}
                      </span>
                    </div>
            ...
            {tab === "why" && (
              <div>
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <span className="text-[9.5px] font-black uppercase tracking-widest text-sky-400">
                    Why It Matters
                  </span>
                </div>
                <p
            ...
            {tab === "how" && (
              <div>
                ...
                {item.apiField.writable && (
                  <div
                    className={`mt-3 flex items-center gap-2 px-3 py-2 rounded-xl border ${dark ? "bg-emerald-500/[0.06] border-emerald-500/20" : "bg-emerald-50 border-emerald-200/60"}`}
                  >
                    <Wand2 size={11} className="text-emerald-400 shrink-0" />
                    <p
                      className={`text-[11px] ${dark ? "text-emerald-300/80" : "text-emerald-700"}`}
                    >
            ...
            {tab === "api" && (
              <div>
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <span className="text-[9.5px] font-black uppercase tracking-widest text-blue-400">
                    Google API Field
                  </span>
                </div>
                <APIBadge field={item.apiField} dark={dark} />
                <div
                  className={`mt-2.5 p-2.5 rounded-xl border ${dark ? "bg-white/[0.02] border-white/[0.05]" : "bg-slate-50 border-slate-100"}`}
                >
                  <p
                    className={`text-[10px] font-mono ${dark ? "text-slate-400" : "text-slate-600"}`}
                    style={{ wordBreak: "break-all" }}
                  >
                ...
                <p
                  className={`text-[11px] mt-2.5 leading-relaxed ${dark ? "text-slate-500" : "text-slate-500"}`}
                >
                  Ref: Google Business Profile API —{" "}
                  <span
                    className={`font-mono ${dark ? "text-blue-400" : "text-blue-600"}`}
                  >
                    {item.googleDoc}
                  </span>
                </p>
            ...
            <div
              className={`flex items-start gap-2.5 p-3 rounded-xl border ${dark ? "bg-[#0d1635] border-blue-800/30" : "bg-blue-50/80 border-blue-200/60"}`}
            >
              <div
                className={`w-6 h-6 rounded-lg flex items-center justify-center shrink-0 ${dark ? "bg-blue-500/20" : "bg-blue-100"}`}
              >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1 mb-0.5">
                  ...
                  <span
                    className={`text-[9px] font-black uppercase tracking-widest ${dark ? "text-blue-400" : "text-blue-600"}`}
                  >
                    AI Insight
                  </span>
                </div>
...
function CategorySection({
  ...
  return (
    <div
      className={`rounded-3xl border overflow-hidden transition-all ${dark ? "bg-[#0a1020] border-blue-900/40" : "bg-white border-blue-100/80 shadow-sm"}`}
      style={{
        boxShadow: dark
          ? `0 0 40px ${cat.color}08`
    ...
    >
      ...
      >
        ...
        <div className="flex-1 text-left min-w-0">
          <div className="flex items-center justify-between mb-1.5 gap-2">
            <p
              className={`text-[13.5px] font-black truncate ${dark ? "text-white" : "text-slate-900"}`}
          ...
          <div
            className={`h-1.5 rounded-full overflow-hidden mb-2 ${dark ? "bg-white/[0.05]" : "bg-blue-50"}`}
          >
            <div
              className="h-full rounded-full transition-all duration-700"
              style={{
          ...
          <div className="flex items-center gap-3">
            ...
            <span
              className={`text-[9.5px] font-medium ml-auto ${dark ? "text-slate-600" : "text-slate-400"}`}
            >
              {pct}% done
            </span>
          </div>
      ...
      {open && (
        <div
          className={`border-t flex flex-col gap-2.5 p-3 ${dark ? "border-blue-900/30" : "border-blue-100/60"}`}
        >
          {cat.items.map((item) => (
            <ItemCard
              key={item.id}
...
function QuickWins({
  ...
  if (!wins.length)
    return (
      <div
        className={`rounded-2xl border px-4 py-3.5 flex items-center gap-3 ${dark ? "bg-emerald-500/[0.06] border-emerald-500/20" : "bg-emerald-50 border-emerald-200/60"}`}
      >
  ...
  return (
    <div
      className={`rounded-2xl border overflow-hidden ${dark ? "bg-[#080f1e] border-blue-900/40" : "bg-white border-blue-100 shadow-sm"}`}
    >
      <div
        className={`px-4 py-3 flex items-center justify-between border-b ${dark ? "border-blue-900/30" : "border-blue-100/60"}`}
      >
        <div className="flex items-center gap-2">
          <div
            className={`w-6 h-6 rounded-lg flex items-center justify-center ${dark ? "bg-orange-500/15" : "bg-orange-100"}`}
          >
      ...
      <div className="p-3 flex flex-col gap-2">
        {wins.map((item, i) => (
          <div
            key={i}
            className={`flex items-center gap-3 p-3 rounded-xl border ${dark ? "bg-white/[0.03] border-white/[0.05]" : "bg-slate-50 border-slate-100"}`}
          >
            <div
            ...
            <div className="flex-1 min-w-0">
              <p
                className={`text-[11.5px] font-bold truncate ${dark ? "text-white" : "text-slate-900"}`}
              >
                {item.title}
              </p>
              <p
                className={`text-[10px] mt-0.5 ${dark ? "text-slate-500" : "text-slate-400"}`}
              >
...
function AIRecs({
  ...
  return (
    ...
    >
      <div
        className={`px-4 py-3 flex items-center gap-2 border-b ${dark ? "border-blue-900/30" : "border-blue-100/60"}`}
      >
        <div
      ...
      <div className="p-3 flex flex-col gap-2">
        {recs.map((r, i) => (
          <div
            key={i}
            className={`flex items-start gap-2.5 p-3 rounded-xl border ${dark ? "bg-white/[0.02] border-white/[0.04]" : "bg-slate-50/70 border-slate-100"}`}
          >
            <div
            ...
            <div className="flex-1 min-w-0">
              <p
                className={`text-[11.5px] font-bold mb-0.5 ${dark ? "text-white" : "text-slate-900"}`}
              >
                {r.title}
              </p>
              <p
                className={`text-[11px] leading-relaxed ${dark ? "text-slate-400" : "text-slate-600"}`}
              >
                {r.text}
              </p>
            </div>
...
export default function ChecklistPage() {
  ...
  const {
    ...
    queryFn: async ({ queryKey }) => {
      ...
      const res = await fetch(`/api/google/checklist-data?locationId=${lid}`, {
        headers: { Authorization: `Bearer ${getToken()}` },
        cache: "no-store",
      });
      const json = await res.json();
  ...
  const isInitialLoading =
    !mounted ||
    userLoading ||
    (!!locationId && (scoreLoading || checklistLoading));
    ...
    <>
      ...
      >
        ...
        <div className="relative max-w-lg mx-auto px-4 pb-32">
            ...
            <div className="flex items-start justify-between">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <div
                    className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[9.5px] font-black uppercase tracking-widest ${dark ? "bg-blue-500/10 border-blue-700 text-blue-400" : "bg-blue-50 border-blue-700 text-blue-600"}`}
                  >
                ...
                <h1
                  className={`text-md font-bold leading-normal ${dark ? "text-white" : "text-slate-900"}`}
                >
                  GBP Score Card
                </h1>
                <p
                  className={`text-[12px] mt-1 ${dark ? "text-slate-500" : "text-blue-700"}`}
                >
                  {scoreData?.meta.locationName ?? "Google Business Profile"} ·
                  30 factors
                </p>
              </div>
              <button
          ...
          {/* ── SCORE HERO CARD ── */}
          <div
            className={`rounded-3xl border mb-5 overflow-hidden ${dark ? "bg-[#070f1f] border-blue-900/50" : "bg-white border-blue-100 shadow-sm"}`}
            style={{
              boxShadow: dark
                ? "0 20px 80px rgba(59,130,246,0.12)"
          ...
          >
            ...
            <div className="p-5">
              ...
              <div
                className={`rounded-2xl p-3 grid grid-cols-3 gap-1 border mt-4 ${dark ? "bg-blue-950/30 border-blue-900/30" : "bg-blue-50/60 border-blue-100"}`}
              >
                {[
            ...
            <div
              className={`border-t px-5 py-4 flex flex-col gap-2.5 ${dark ? "border-blue-900/30" : "border-blue-100/60"}`}
            >
              <p
                className={`text-[10px] font-black uppercase tracking-widest mb-1 ${dark ? "text-slate-600" : "text-slate-400"}`}
              >
                All Categories
              </p>
            ...
            <div
              className={`border-t grid grid-cols-5 ${dark ? "border-blue-900/30" : "border-blue-100/60"}`}
            >
              {[
                {
                  label: "Incomplete",
                ...
                <div
                  key={i}
                  className={`flex flex-col items-center py-3 gap-0.5 border-r last:border-r-0 ${dark ? "border-blue-900/30" : "border-blue-100/60"} ${tier.active ? (dark ? "bg-white/[0.03]" : "bg-blue-50/60") : ""}`}
                >
                  <div
                  ...
                  <span
                    className={`text-[8px] ${dark ? "text-slate-700" : "text-slate-300"}`}
                  >
                    {tier.range}
                  </span>
                </div>
          ...
          {scoreData?.meta && (
            <div
              className={`rounded-2xl border p-4 mb-5 ${dark ? "bg-[#080f1e] border-blue-900/40" : "bg-white border-blue-100 shadow-sm"}`}
            >
              <p
                ...
                {[
                  {
                    label: "Avg Rating",
                    value:
                      scoreData.meta.avgRating > 0
                        ? `${scoreData.meta.avgRating}★`
                        : "—",
                    color: "#f59e0b",
                  },
                  {
                  ...
                  <div
                    key={i}
                    className={`flex flex-col items-center gap-0.5 py-2.5 px-2 rounded-xl border ${dark ? "bg-white/[0.03] border-white/[0.05]" : "bg-slate-50 border-slate-100"}`}
                  >
                    <span
          ...
          <div className="mb-5">
            <div className="flex items-center justify-between mb-3">
              <p
                className={`text-[14px] font-black ${dark ? "text-white" : "text-slate-900"}`}
                style={{ letterSpacing: "-0.03em" }}
              >
                Quick Wins
              </p>
              <span
                className={`text-[11px] font-semibold ${dark ? "text-slate-500" : "text-blue-400"}`}
              >
              ...
              return (
                <button
                  ...
                  style={
                    active
                      ? {
                          background: `${colors[f]}15`,
                          color: colors[f],
                          borderColor: `${colors[f]}35`,
                        }
                      : {
                          background: dark
          ...
          <div className="mt-6">
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-4">
                <div
                  className={`w-11 h-11 rounded-2xl flex items-center justify-center ${dark ? "bg-blue-500/15" : "bg-blue-100"}`}
                ...
                <div>
                  ...
                  <p
                    className={`text-[11px] mt-0.5 ${dark ? "text-slate-500" : "text-blue-400"}`}
                  >
              ...
              <div className="grid grid-cols-2 gap-2 mb-4">
                {[
                  ...
                  <div
                    key={i}
                    className={`flex items-center gap-2 px-3 py-2 rounded-xl border ${dark ? "bg-white/[0.03] border-white/[0.05]" : "bg-slate-50 border-slate-100"}`}
                  >
                    <span style={{ color: f.color }}>{f.icon}</span>
                    <span
                      className={`text-[10.5px] font-semibold ${dark ? "text-slate-300" : "text-slate-600"}`}
                    >
                      {f.label}
                    </span>
                  </div>
              ...
              <p
                className={`text-center text-[10px] mt-3 ${dark ? "text-slate-600" : "text-slate-400"}`}
              >
                Real data from Google Business Profile API · Score updates on
                Re-scan
              </p>
            </div>
          </div>
