...
// mobile_app/app/(main)/reviews/google/page.tsx
//
// CHANGE: Added <AIReplyModal /> — shown when user taps "Auto-Reply with AI"
// The modal replaces the plain loading state with:
//   • Animated glowing brain / SEO expert persona
//   • Live progress bar
//   • "Currently processing" card (active review)
//   • Completed items feed with ✓ / ✗ status
//   • Typewriter status messages
//   • Subtle pulsing particles background
...
import {
  ...
  WifiOff, Building2, X, Zap, Brain, Shield,
  BarChart2, Target, Award, ChevronRight,
} from "lucide-react";
...
interface ReviewsResponse {
  success: boolean; total: number; page: number; limit: number;
  totalPages: number; reviews: GoogleReview[]; error?: string;
}
...
const normalise   = (g: GoogleReview): Review => {
  ...
  return {
    ...
    author: g.reviewer.displayName, initials: mkInitials(g.reviewer.displayName),
    rating, date: new Date(g.createTime).toLocaleDateString("en-IN",{day:"numeric",month:"short",year:"numeric"}),
...
const TEMPLATES: Record<Review["sentiment"], ((r: Review) => string)[]> = {
  ...
  neutral:  [r => `Thank you for your honest feedback, ${r.author.split(" ")[0]}. We appreciate your time and are always looking for ways to improve. We'd love for you to give us another chance!`],
  negative: [
...
function AIReplyModal({
  items, isDark, onClose, isComplete,
}: {
  ...
  return (
    <div className="fixed inset-0 z-60 flex flex-col items-center" style={{ fontFamily: "-apple-system,'SF Pro Text',sans-serif" }}>
      {/* backdrop */}
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <style>{`
          ...
          @keyframes typewriter{0%,100%{opacity:1}50%{opacity:0.4}}
        `}</style>
      </div>

      {/* panel */}
      <div
        className={`relative w-full max-w-[460px] mx-auto mt-auto rounded-t-[28px] overflow-hidden max-h-[90vh] flex flex-col`}
        style={{ background: isDark ? "#070f1f" : "#fff", boxShadow: "0 -24px 80px rgba(0,0,0,0.6), inset 0 1px 0 rgba(59,130,246,0.2)" }}
      >
        {/* top accent */}
        <div className="h-[1.5px]" style={{ background: "linear-gradient(90deg,transparent,#3b82f6,#60a5fa,transparent)" }}/>
        ...
        <div className="px-5 pt-2 pb-4 flex items-start gap-4">
          ...
          <div className="relative shrink-0 flex items-center justify-center" style={{ width: 64, height: 64 }}>
            ...
            <div className="w-14 h-14 rounded-full flex items-center justify-center"
              style={{ background: isComplete ? "linear-gradient(135deg,#16a34a,#22c55e)" : "linear-gradient(135deg,#1e40af,#3b82f6)", boxShadow: isComplete ? "0 0 24px rgba(34,197,94,0.5)" : "0 0 24px rgba(59,130,246,0.5)" }}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-0.5">
              <p className={`text-[17px] font-black ${isDark ? "text-white" : "text-slate-900"}`} style={{ letterSpacing: "-0.03em" }}>
                {isComplete ? "All Done!" : "SEO AI Agent"}
              </p>
              {!isComplete && (
                <div className="flex items-center gap-1 px-2 py-0.5 rounded-full" style={{ background: "rgba(59,130,246,0.12)", border: "1px solid rgba(59,130,246,0.25)" }}>
        ...
        <div className="px-5 mb-4">
          ...
          <div className={`h-2.5 rounded-full overflow-hidden ${isDark ? "bg-white/[0.06]" : "bg-slate-100"}`}>
            <div className="h-full rounded-full relative overflow-hidden transition-none"
              style={{
                width: `${barWidth}%`,
        ...
        {!isComplete && (
          <div className="px-5 mb-3">
            <div className={`flex items-center gap-2.5 px-3.5 py-2.5 rounded-2xl border ${isDark ? "bg-blue-500/[0.06] border-blue-900/50" : "bg-blue-50 border-blue-200/60"}`}>
              ...
              <p className={`text-[11.5px] font-semibold flex-1 ${isDark ? "text-blue-300" : "text-blue-700"}`}
                style={{ animation: "typewriter 0.3s ease" }}>
                {SEO_PHASES[phaseIdx]}
              </p>
        ...
        {/* SEO badges — what the AI is doing */}
        {!isComplete && (
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-6 flex flex-col gap-2.5" style={{ scrollbarWidth: "none" }}>
          ...
          {active && (
            <div className="mb-1">
              ...
              <div className={`rounded-2xl border overflow-hidden ${isDark ? "bg-[#0d1c36] border-blue-900/50" : "bg-blue-50 border-blue-200/60"}`}
                ...
                {/* top shimmer */}
                <div className="h-[1.5px]" style={{ background: "linear-gradient(90deg,transparent,#3b82f6,transparent)", animation: "shimmer 1.5s linear infinite" }}/>
                <div className="p-3.5">
                  ...
                  {active.reply && (
                    <div className={`mt-2 px-3 py-2 rounded-xl border-l-2 border-blue-500 ${isDark ? "bg-white/[0.03]" : "bg-white/50"}`}>
                      ...
                      <p className={`text-[11px] leading-relaxed line-clamp-3 ${isDark ? "text-slate-300" : "text-slate-700"}`}>
                        {active.reply}
                      </p>
                    </div>
                  )}
              ...
              <div className="flex flex-col gap-2">
                {[...completed].reverse().map(item => (
                  <div key={item.reviewId}
                    className={`flex items-start gap-3 px-3.5 py-3 rounded-2xl border ${isDark ? "bg-white/[0.025] border-white/[0.05]" : "bg-white border-slate-100"}`}>
                    {/* avatar */}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {item.reply && (
                        <p className={`text-[10px] leading-relaxed line-clamp-2 ${isDark ? "text-slate-500" : "text-slate-400"}`}>
                          {item.reply}
                        </p>
                      )}
                    </div>
          ...
          {/* ── COMPLETE STATE ── */}
          {isComplete && (
            <div className={`rounded-2xl border p-4 flex flex-col items-center text-center ${isDark ? "bg-emerald-500/[0.06] border-emerald-900/40" : "bg-emerald-50 border-emerald-200/60"}`}>
              <div className="flex gap-3 mb-3">
                {[
                  ...
                  <div key={i} className="flex flex-col items-center gap-1">
                    <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: `${b.color}15`, color: b.color }}>{b.icon}</div>
...
/* ══════════════════════════════════════════════════════════
   ATOMS  (unchanged from original)
══════════════════════════════════════════════════════════ */
function StarRow({ rating, size = 12 }: { rating: number; size?: number }) {
  return (
    <div className="flex gap-0.5">
      {[1,2,3,4,5].map(i => <Star key={i} size={size} strokeWidth={0} fill={i <= rating ? "#FBBF24" : "#374151"}/>)}
    </div>
  );
}
...
function StatCard({ label, value, sub, icon, color, isDark }: {
  ...
  return (
    <div className={`rounded-2xl p-4 flex flex-col gap-2 border ${isDark ? "bg-[#131c2d] border-white/[0.06]" : "bg-white border-black/[0.05] shadow-sm"}`}>
      <div className="flex items-center justify-between">
        ...
        <span className="p-1.5 rounded-lg" style={{ background: `${color}18` }}><span style={{ color }}>{icon}</span></span>
      </div>
      <div>
        <div className={`text-2xl font-black tracking-tight ${isDark ? "text-white" : "text-slate-900"}`}
          style={{ fontFamily: "-apple-system,'SF Pro Display',sans-serif", letterSpacing: "-0.04em" }}>{value}</div>
...
function ReplyComposer({ review, isDark, onSend, onClose }: {
  ...
  const MAX = 4096; const taRef   = useRef<HTMLTextAreaElement>(null);
  ...
  const generateAI = async (tone?: string) => {
    try {
      ...
      const res  = await fetch("/api/ai/reply", {
        ...
        body: JSON.stringify({ review: review.text ?? "", rating: review.rating, reviewerName: review.author, businessName: user?.googleLocationName, tone: tone || "Professional" }),
      });
      const json = await res.json();
      if (!json.success) throw new Error(json.error || "AI generation failed");
      setText(json.reply); taRef.current?.focus();
  ...
  const handleSend = async () => {
    ...
    setPosting(true);
    try { await onSend(text.trim()); } catch (e: any) { setApiError(e.message ?? "Failed to post reply."); }
    finally { setPosting(false); }
  };
  const remaining = MAX - text.length; const overLimit = remaining < 0;
    ...
    <div className={`mt-3 rounded-2xl p-4 border ${isDark ? "bg-[#0d1421] border-white/[0.08]" : "bg-slate-50 border-black/[0.06]"}`}>
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        <button onClick={() => generateAI()} disabled={generating}
          className={`flex items-center gap-1.5 h-8 px-3 rounded-xl text-[12px] font-semibold transition-all active:scale-95 disabled:opacity-60 ${isDark ? "bg-blue-500/15 text-blue-400 border border-blue-500/25" : "bg-blue-50 text-blue-600 border border-blue-200"}`}>
        ...
        {["Professional","Friendly","Empathetic"].map(tone => (
          <button key={tone} onClick={() => generateAI(tone)} disabled={generating}
            className={`h-7 px-2.5 rounded-lg text-[11px] font-medium transition-all active:scale-95 disabled:opacity-50 ${isDark ? "bg-white/[0.06] text-slate-400" : "bg-white text-slate-500 border border-slate-200"}`}>
            {tone}
          </button>
        ))}
      </div>
      <div className="relative">
        <textarea ref={taRef} value={text} rows={14}
      ...
      <div className="flex flex-col items-center justify-between mt-2 px-0.5">
        <div>
        ...
        <div className="flex justify-center w-full gap-2">
          <button onClick={onClose} className={`h-8 px-3 rounded-xl text-[12px] font-semibold transition-all ${isDark ? "text-slate-500" : "text-slate-400"}`}>Cancel</button>
      ...
      <div className="p-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                {review.flagged && <span className={`text-[10px] font-semibold border rounded-full px-2 py-0.5 ${isDark ? "bg-red-500/15 text-red-400 border-red-500/20" : "bg-red-50 text-red-500 border-red-200"}`}>Flagged</span>}
                <span className={`text-[10px] font-semibold border rounded-full px-2 py-0.5 ${sentimentCls(review.sentiment, isDark)}`}>{review.sentiment}</span>
              </div>
            </div>
        ...
        {review.text ? (
          <p className={`text-[13.5px] leading-relaxed ${isDark ? "text-slate-300" : "text-slate-700"}`}
            ...
            {needsTrunc && <button onClick={() => setExpanded(v => !v)} className="ml-1 text-blue-500 text-[12px] font-medium">{expanded ? "Less" : "More"}</button>}
          </p>
        ) : (
        ...
        <div className={`flex items-center gap-1 mt-3 pt-3 border-t ${isDark ? "border-white/[0.05]" : "border-slate-100"}`}>
          <button onClick={() => setOpen(v => !v)}
            className={`flex items-center gap-1.5 h-7 px-3 rounded-xl text-[12px] font-semibold transition-all active:scale-95 ${open ? "bg-blue-500 text-white" : isDark ? "bg-white/[0.07] text-slate-300" : "bg-slate-100 text-slate-600"}`}>
            <MessageSquare size={12}/>{review.replied ? "Edit Reply" : "Reply"}
          </button>
          <button onClick={() => onHelpful(review.id)} className={`flex items-center gap-1.5 h-7 px-3 rounded-xl text-[12px] font-medium transition-all active:scale-95 ${isDark ? "text-slate-500" : "text-slate-400"}`}>
            <ThumbsUp size={12}/>{review.helpful}
          </button>
          <button onClick={() => onFlag(review.id)}
            className={`ml-auto flex items-center gap-1 h-7 px-2.5 rounded-xl text-[11px] font-medium transition-all active:scale-95 ${review.flagged ? (isDark ? "text-red-400 bg-red-500/10" : "text-red-500 bg-red-50") : (isDark ? "text-slate-600" : "text-slate-400")}`}>
...
export default function GoogleReviewsPage() {
  ...
  const fetchPage = useCallback(async (pageNum: number, append = false) => {
    ...
    try {
      ...
      const res   = await fetch(`/api/google/reviews?location=accounts/me/locations/${user.googleLocationId}&page=${pageNum}`,
      ...
      setTotalCount(json.total); setTotalPages(json.totalPages); setPage(json.page);
    } catch (e: any) { setFetchErr(e.message ?? "Something went wrong"); }
  ...
  const bulkAI = async () => {
    ...
    const initial: AIItem[] = unreplied.map(r => ({
      reviewId: r.id, author: r.author, rating: r.rating, text: r.text,
      status: "pending",
    }));
    setAiItems(initial);
    setAiComplete(false);
    ...
    for (let i = 0; i < unreplied.length; i++) {
      ...
      try {
        ...
        // → posting
        setAiItems(prev => prev.map(item =>
          item.reviewId === r.id ? { ...item, status: "posting", reply: replyText } : item
        ));
        ...
        // → done
        setAiItems(prev => prev.map(item =>
          item.reviewId === r.id ? { ...item, status: "done", reply: replyText } : item
        ));
      } catch {
        setAiItems(prev => prev.map(item =>
      ...
      <div className="max-w-lg mx-auto px-4 pb-28">
        ...
        <div className="pt-4 pb-5">
          <div className="flex items-center justify-between">
            ...
            <button onClick={refresh} disabled={fetching}
              className={`w-9 h-9 flex items-center justify-center rounded-xl transition-all active:scale-90 disabled:opacity-50 ${isDark ? "bg-white/[0.07] text-slate-400" : "bg-white text-slate-500 border border-slate-200"}`}>
        ...
        {!userLoading && !user?.googleLocationId && (
          <div className={`rounded-2xl p-8 text-center border ${isDark ? "bg-[#131c2d] border-white/[0.06]" : "bg-white border-black/[0.05]"}`}>
            ...
            <p className={`text-[12.5px] ${isDark ? "text-slate-500" : "text-slate-400"}`}>Go to your Profile page and link your GBP to start managing reviews.</p>
          </div>
        )}
        ...
        {!isInitialLoad && reviews.length > 0 && (
          <>
            ...
            {/* rating breakdown */}
            <div className={`rounded-2xl p-4 mb-4 border ${isDark ? "bg-[#131c2d] border-white/[0.06]" : "bg-white border-black/[0.05] shadow-sm"}`}>
              <div className="flex items-center justify-between mb-3">
                <div>
                  <span className={`text-[11px] font-semibold uppercase tracking-[0.07em] ${isDark ? "text-slate-500" : "text-slate-400"}`}>Rating Breakdown</span>
                  <span className={`ml-2 text-[10px] ${isDark ? "text-slate-600" : "text-slate-400"}`}>({loaded} of {totalReviewsFromAnalytics} loaded)</span>
                </div>
                <div className="flex items-center gap-1">
                  <Star size={13} fill="#FBBF24" strokeWidth={0}/>
                  <span className={`text-[15px] font-black ${isDark ? "text-white" : "text-slate-900"}`} style={{ letterSpacing: "-0.03em" }}>{avg}</span>
                </div>
              </div>
            ...
            {unreplied > 0 && (
              <button onClick={bulkAI}
                className="w-full h-12 rounded-[14px] flex items-center justify-center gap-2.5 text-[13.5px] font-bold text-white mb-4 transition-all active:scale-[0.97] relative overflow-hidden"
            ...
            {/* search */}
            <div className={`flex items-center gap-2.5 h-[42px] px-3.5 rounded-[13px] border mb-3 ${isDark ? "bg-[#131c2d] border-white/[0.07]" : "bg-white border-black/[0.07]"}`}>
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search reviews…"
                className={`flex-1 bg-transparent outline-none text-[13.5px] ${isDark ? "text-white placeholder:text-slate-600" : "text-slate-900 placeholder:text-slate-400"}`}/>
              {search && <button onClick={() => setSearch("")} className={`text-[11px] font-medium ${isDark ? "text-slate-500" : "text-slate-400"}`}>Clear</button>}
            </div>
            ...
            <div className="flex items-center gap-2 mb-4">
              <div className="flex gap-1.5 flex-1 overflow-x-auto no-scrollbar">
                {(["all","unreplied","replied","positive","negative"] as FilterType[]).map(f => (
                  <button key={f} onClick={() => setFilter(f)}
                    className={`h-8 px-3 rounded-xl text-[12px] font-semibold whitespace-nowrap shrink-0 transition-all active:scale-95 ${filter === f ? "bg-blue-500 text-white" : isDark ? "bg-white/[0.07] text-slate-400" : "bg-white text-slate-500 border border-slate-200"}`}>
              ...
              <div className="relative shrink-0">
                <button onClick={() => setShowSort(v => !v)}
                  className={`flex items-center gap-1 h-8 px-3 rounded-xl text-[12px] font-semibold transition-all active:scale-95 ${isDark ? "bg-white/[0.07] text-slate-400" : "bg-white text-slate-500 border border-slate-200"}`}>
                  <Filter size={11}/>{SORT_LABELS[sort]}
                  <ChevronDown size={11} className={`transition-transform ${showSort ? "rotate-180" : ""}`}/>
                </button>
                {showSort && (
                  <div className={`absolute right-0 top-10 z-20 rounded-2xl border overflow-hidden shadow-xl min-w-[140px] ${isDark ? "bg-[#131c2d] border-white/[0.08]" : "bg-white border-black/[0.06]"}`}>
                    {(Object.entries(SORT_LABELS) as [SortType, string][]).map(([k,v]) => (
                      <button key={k} onClick={() => { setSort(k); setShowSort(false); }}
                        className={`w-full text-left px-4 py-2.5 text-[13px] transition-colors ${sort === k ? "text-blue-500 font-semibold" : isDark ? "text-slate-300 hover:bg-white/[0.06]" : "text-slate-700 hover:bg-slate-50"}`}>
                        {v}
                      </button>
                    ))}
                  </div>
            ...
            <div className="mt-5">
              ...
              {!loadingMore && hasMore && (
                <button onClick={loadMore}
                  className={`w-full h-11 rounded-[13px] flex items-center justify-center gap-2 text-[13px] font-semibold border transition-all active:scale-[0.97] ${isDark ? "bg-[#131c2d] border-white/[0.08] text-slate-300" : "bg-white border-slate-200 text-slate-600"}`}>
                  Load More Reviews
                </button>
              )}
