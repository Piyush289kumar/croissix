...
function AIImageCard({
  ...
  return (
    <div className={card}>
      {/* header */}
      <div
        className={`px-4 py-2.5 flex items-center gap-2 border-b ${dark ? "border-[#1a2d4a]" : "border-slate-100"}`}
      >
        <div
      ...
      <div className="p-3 flex flex-col gap-3">
        ...
        <div>
          <p
            className={`text-[9px] font-black uppercase tracking-widest mb-2 ${dark ? "text-slate-700" : "text-slate-400"}`}
          >
            Image Style
          </p>
          <div
        ...
        {generating ? (
          ...
          <div
            className={`w-full rounded-2xl overflow-hidden relative ${dark ? "bg-[#0d1c36]" : "bg-slate-100"}`}
          ...
          >
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
              ...
              >
                {[1, 2].map((i) => (
                  <div
                    ...
                    style={{
                      animation: `pulse-ring ${1.2 + i * 0.5}s ease-out infinite`,
              ...
              <div className="flex flex-col items-center gap-1.5">
                ...
                <p
                  className={`text-[9.5px] ${dark ? "text-slate-600" : "text-slate-400"}`}
                >
                  Using FLUX AI model
                </p>
              </div>
          ...
          >
            ...
            >
              <p className="text-white text-[10px] font-medium flex-1 truncate opacity-70">
        ...
        {imgError && (
          <div
            className={`flex items-start gap-2 px-3 py-2.5 rounded-xl border ${dark ? "bg-red-500/[0.07] border-red-900/40" : "bg-red-50 border-red-200/60"}`}
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              >
                Add <code className="font-mono">TOGETHER_API_KEY</code> to .env
...
function SEOMeter({
  ...
  return (
    <div
      className={`rounded-2xl border overflow-hidden ${dark ? "bg-[#080f1e] border-blue-900/40" : "bg-white border-slate-200/80 shadow-sm"}`}
    >
      <button
      ...
      {open && tips.length > 0 && (
        <div
          className={`border-t px-4 py-3 flex flex-col gap-2 ${dark ? "border-blue-900/30" : "border-slate-100"}`}
        >
          {tips.map((tip, i) => (
            <div key={i} className="flex items-start gap-2">
              <Lightbulb size={11} className="text-amber-400 shrink-0 mt-0.5" />
              <p
                className={`text-[11px] ${dark ? "text-slate-400" : "text-slate-600"}`}
              >
                {tip}
              </p>
            </div>
...
function KeywordPanel({
  ...
  return (
    ...
    >
      <div
        className={`px-4 py-2.5 flex items-center gap-2 border-b ${dark ? "border-blue-900/30" : "border-slate-100"}`}
      >
        ...
        <span
          className={`ml-auto text-[9.5px] font-bold ${dark ? "text-slate-600" : "text-slate-400"}`}
        >
      ...
      <div className="p-3 flex flex-col gap-2.5">
        {embedded.length > 0 && (
          <div>
            <p
              className={`text-[9px] font-black uppercase tracking-widest mb-1.5 ${dark ? "text-slate-700" : "text-slate-400"}`}
            >
              In Your Post
            </p>
            ...
            <div className="flex flex-wrap gap-1.5">
              {suggested.map((kw) => (
                ...
                >
                  ...
                  />
                  <span
                    className={`text-[10px] font-bold ${dark ? "text-slate-400" : "text-slate-600"}`}
                  >
                    {kw}
                  </span>
                </button>
        ...
        <div
          className={`flex items-center gap-2 h-8 px-2.5 rounded-xl border ${dark ? "bg-white/[0.03] border-white/[0.06]" : "bg-slate-50 border-slate-200"}`}
        >
          <Plus
            size={10}
          ...
          {custom.trim() && (
            <button
              ...
              className="text-blue-400 text-[9.5px] font-black"
            >
              Add
            </button>
          )}
...
function PageSkeleton({ isDark }: { isDark: boolean }) {
  return (
    <div className="flex flex-col gap-4 px-4 pt-4 pb-28">
      ...
      <Skeleton isDark={isDark} className="h-16 rounded-2xl" />
      <Skeleton isDark={isDark} className="h-40 rounded-2xl" />
      <Skeleton isDark={isDark} className="h-[200px] rounded-2xl" />
...
function UploadProgress({
  ...
  return (
    <div
      className={`rounded-2xl p-4 border flex flex-col gap-2.5 mb-4 ${isDark ? "bg-blue-500/[0.06] border-blue-500/20" : "bg-blue-50 border-blue-200"}`}
    >
...
function StepBadge({
  ...
  return (
    <div
      className={`flex items-center gap-3 p-3 rounded-2xl mb-4 border ${isDark ? "bg-blue-500/[0.07] border-blue-500/20" : "bg-blue-50 border-blue-200"}`}
    >
      ...
      <div className="flex-1">
        <div className="flex gap-1.5 mb-1.5">
          {steps.map((s, i) => (
            <div
              key={s.id}
              className={`h-1 rounded-full transition-all duration-500 ${step === s.id ? "w-8 bg-blue-500" : steps.indexOf(steps.find((x) => x.id === step)!) > i ? "w-4 bg-green-500" : "w-4 bg-white/[0.1]"}`}
            />
          ))}
        </div>
        <p
  ...
  return (
    ...
    >
      ...
      >
        <button
          onClick={prev}
          className={`w-7 h-7 flex items-center justify-center rounded-lg ${isDark ? "text-slate-400" : "text-slate-500"}`}
        >
          <ChevronLeft size={15} />
        </button>
        <span
            ...
            <div key={i}>
              {d === null ? (
                ...
                <button
                  ...
                  className={`w-full aspect-square flex items-center justify-center rounded-xl text-[12px] font-medium transition-all active:scale-90 disabled:opacity-25 ${isSelected(d) ? "bg-blue-500 text-white" : isToday(d) ? (isDark ? "bg-blue-500/20 text-blue-400" : "bg-blue-50 text-blue-600") : isDark ? "text-slate-300" : "text-slate-700"}`}
                >
                  {d}
                </button>
              )}
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1.5 ml-auto">
            <select
              ...
              className={`h-8 px-2 rounded-lg text-[13px] font-medium outline-none border ${isDark ? "bg-[#182236] border-white/[0.07] text-white" : "bg-slate-50 border-black/[0.07] text-slate-900"}`}
            >
      ...
      <div className="px-4 pb-3 pt-1 flex gap-2">
        <button
          onClick={onClose}
          className={`flex-1 h-9 rounded-xl text-[13px] font-semibold ${isDark ? "bg-white/[0.07] text-slate-300" : "bg-slate-100 text-slate-600"}`}
        >
          Cancel
        </button>
        <button
    ...
    <div className={disabled ? "pointer-events-none opacity-50" : ""}>
      {images.length > 0 ? (
        ...
        >
          ...
          />
          <p
            className={`text-[12px] font-semibold ${isDark ? "text-slate-400" : "text-slate-500"}`}
          >
            Upload from device
          </p>
        </button>
    ...
    >
      ...
      >
        <div className="p-4">
          ...
          {cta !== "NONE" && (
            <div
              className={`inline-flex items-center gap-1.5 h-8 px-3 rounded-full text-[12px] font-semibold ${isDark ? "bg-blue-500/20 text-blue-400" : "bg-blue-50 text-blue-600"}`}
            >
        ...
        >
          <button
            onClick={onClose}
            className={`w-full h-10 rounded-2xl text-[13px] font-semibold ${isDark ? "bg-white/[0.07] text-slate-300" : "bg-slate-100 text-slate-600"}`}
          >
            Close Preview
          </button>
        </div>
  ...
  const { data: recentPostsData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(
        `/api/google/posts?location=accounts/me/locations/${user.googleLocationId}`,
        {
    ...
    >
      <div className="max-w-lg mx-auto px-4 pb-28">
              ...
              className={`flex flex-col items-center gap-1.5 py-3.5 px-2 rounded-2xl border transition-all active:scale-95 disabled:opacity-50
                ${
                  postType === t.id
                    ...
                    : isDark
                      ? "bg-[#131c2d] border-white/[0.06] text-slate-400"
                      : "bg-white border-black/[0.05] text-slate-500 shadow-sm"
                }`}
            >
              ...
              <span
                className={`text-[10px] text-center leading-tight ${postType === t.id ? (isDark ? "text-blue-500/70" : "text-blue-500/70") : isDark ? "text-slate-600" : "text-slate-400"}`}
              >
                {t.desc}
              </span>
            </button>
        ...
        <div className={`${card} mb-4 overflow-hidden`}>
          <div
            className={`px-4 py-3 flex items-center gap-2 border-b ${isDark ? "border-white/[0.05]" : "border-slate-100"}`}
          >
            <div
              className={`w-6 h-6 rounded-lg flex items-center justify-center ${isDark ? "bg-blue-500/15" : "bg-blue-100"}`}
            >
              <Type size={12} className="text-blue-400" />
            </div>
            <p
              className={`text-[12.5px] font-black ${isDark ? "text-white" : "text-slate-900"}`}
            >
              Post Title
            </p>
            <span
              className={`ml-auto text-[10px] font-bold ${isDark ? "text-slate-600" : "text-slate-400"}`}
            >
              Step 1 of 2
            </span>
          </div>
          <div className="p-4">
            <div
              className={`flex items-center gap-2.5 h-12 px-3.5 rounded-2xl border mb-3 transition-all ${isDark ? "bg-[#0d1421] border-white/[0.07] focus-within:border-blue-500/50" : "bg-slate-50 border-black/[0.07] focus-within:border-blue-400"}`}
            >
              <Tag
                size={14}
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" && postTitle.trim()) generateBoth();
                }}
                disabled={isSubmitting}
                placeholder="e.g. Summer Sale 50% Off · New Menu Launch · Grand Opening"
            ...
            <button
              ...
              style={{
                background: "linear-gradient(135deg,#1e3a6e,#2563eb,#60a5fa)",
                boxShadow: postTitle.trim()
            ...
            >
              ...
              <div className="relative flex items-center gap-2.5">
                {isGenerating ? (
                  ...
                  <>
                    <GeminiSparkle size={14} /> Generate SEO Post + Image
                  </>
                )}
              </div>
            </button>
            ...
            {aiError && (
              <div
                className={`mt-2.5 flex items-center gap-2 px-3 py-2 rounded-xl border ${isDark ? "bg-red-500/[0.07] border-red-900/40" : "bg-red-50 border-red-200/60"}`}
              >
                <AlertCircle size={12} className="text-red-400 shrink-0" />
                <p
                  className={`text-[11px] ${isDark ? "text-red-400" : "text-red-600"}`}
                >
                  {aiError}
                </p>
              </div>
        ...
        <div className={`${card} mb-4 overflow-hidden`}>
          <div
            className={`px-4 py-2.5 flex items-center gap-2 border-b ${isDark ? "border-white/[0.05]" : "border-slate-100"}`}
          >
            <div
              className={`w-6 h-6 rounded-lg flex items-center justify-center ${isDark ? "bg-emerald-500/15" : "bg-emerald-100"}`}
            >
          ...
          <div
            className={`flex items-center justify-between px-4 pb-3 pt-0 gap-2 border-t ${isDark ? "border-white/[0.04]" : "border-slate-100"}`}
          >
            <span
              className={`text-[11px] font-medium ${overLimit ? "text-red-400" : remaining < 100 ? "text-orange-400" : isDark ? "text-slate-600" : "text-slate-400"}`}
            >
              {overLimit
        ...
        {duplicateOpener && !isGenerating && (
          <div
            className={`mb-4 flex items-start gap-2.5 px-3.5 py-3 rounded-2xl border ${isDark ? "bg-amber-500/[0.06] border-amber-900/40" : "bg-amber-50 border-amber-200/60"}`}
          >
            <AlertCircle size={13} className="text-amber-400 shrink-0 mt-0.5" />
            <p
              className={`text-[11px] leading-relaxed ${isDark ? "text-amber-400/80" : "text-amber-700"}`}
            >
              <span className="font-black">Similar opening detected.</span> Your
              recent posts also start with "{duplicateOpener}" — consider
              editing the first sentence.
            </p>
          </div>
        )}
        ...
        <div className={`${card} mb-4 p-4`}>
          ...
          {images.length > 0 && (
            <p
              className={`text-[10px] mt-2 ${isDark ? "text-slate-700" : "text-slate-400"}`}
            >
        ...
        <div className={`${card} mb-4 p-4`}>
          ...
          {showCtaUrl && cta !== "NONE" && cta !== "CALL" && (
            <div
              className={`mt-3 flex items-center gap-2 h-10 px-3 rounded-xl border ${isDark ? "bg-[#182236] border-white/[0.07]" : "bg-slate-50 border-black/[0.07]"}`}
            >
              <Link2
                size={13}
                className={isDark ? "text-slate-600" : "text-slate-400"}
        ...
        <div className={`${card} mb-4 overflow-hidden`}>
          ...
          >
            <div
              className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 ${schedule ? (isDark ? "bg-orange-500/20" : "bg-orange-50") : isDark ? "bg-white/[0.07]" : "bg-slate-100"}`}
            >
              <Calendar
                size={15}
                className={
            ...
            <div className="flex-1 text-left">
              ...
              <p
                className={`text-[11px] ${schedule ? (isDark ? "text-orange-400/80" : "text-orange-500") : isDark ? "text-slate-600" : "text-slate-400"}`}
              >
                {schedule
            ...
            <div className="flex items-center gap-2">
              {schedule && (
                <button
                  ...
                  className={`w-6 h-6 flex items-center justify-center rounded-lg ${isDark ? "text-slate-500" : "text-slate-400"}`}
                >
                  <X size={13} />
                </button>
              )}
          ...
          {showCal && (
            <div
              className={`border-t p-3 ${isDark ? "border-white/[0.05]" : "border-slate-100"}`}
            >
              <CalendarPicker
                value={calDraft}
                onChange={(v) => {
        ...
        {displayError && (
          <div
            className={`flex items-start gap-2.5 p-3.5 rounded-2xl mb-4 border ${isDark ? "bg-red-500/[0.08] border-red-500/20" : "bg-red-50 border-red-200"}`}
          >
            ...
            <div className="flex-1">
              ...
              {postMutation.isError && (
                <button
                  ...
                  className="text-[11px] font-semibold text-blue-500 mt-1"
                >
                  Dismiss
                </button>
              )}
